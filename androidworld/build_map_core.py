...
Usage:
    ...
    from aworld_agent.and_controller import AndroidController
...
from aworld_agent.app_map import make_vertical_scroll_step
from aworld_agent.llm_client import text_completion
...
def build_app_map(
    ...
    # -----------------------------------------------------------------------
    def _execute_swipe_step(step: dict):
        """Execute one serialized swipe exactly as macro replay will."""
        controller.run_command(
            f"adb shell input swipe {step['x1']} {step['y1']} "
            f"{step['x2']} {step['y2']} {step.get('duration', 400)}"
        )
        time.sleep(step.get("wait", 0.5))

    def _scroll_steps(count: int) -> list[dict]:
        return [
            make_vertical_scroll_step(screen_w, screen_h, wait=1.0)
            for _ in range(max(0, count))
        ]

    def _scroll_explore(prefix: str, tmp_dir: str) -> list[dict]:
        """Scroll and capture elements across multiple viewport positions.

        Each element records the minimum number of canonical swipes needed to
        make its saved coordinates valid.  The same serialized swipe is later
        embedded in screen macros and replayed by ``AppMap``.
        """
        all_elements = []
        found_texts = {}
        seen_elements = set()
        ...
        for page in range(scroll_pages + 1):
            ...
            # A repeated viewport means the previous swipe did not move the
            # page (non-scrollable screen or already at the bottom).
            current_view = tuple(sorted(
                (e["text"], round(e["nx"], 4), round(e["ny"], 4))
                for e in page_elements
            ))
            if page > 0 and current_view == previous_view:
                break
            ...
            for e in page_elements:
                ...
                # before deduplication so fixed headers can be detected.
                text = e["text"]
                ...
                # Different rows commonly occupy the same screen coordinate
                # after a scroll.  Deduplicating on coordinates alone drops
                # precisely the newly revealed elements we need to retain.
                element_key = (text, e["nx"], e["ny"])
            ...
            if page < scroll_pages:
                _execute_swipe_step(make_vertical_scroll_step(
                    screen_w, screen_h, wait=1.0,
                ))
    ...
    # Setup
    # -----------------------------------------------------------------------
    # Parallel builders may share one output directory.  Isolate crawler XML
    # by output filename so screen0.xml from one emulator cannot overwrite a
    # different app's capture on another emulator.
    output_parent = os.path.dirname(output_path) or "."
    tmp_root = os.path.join(output_parent, ".build_tmp")
    tmp_name = re.sub(r"[^A-Za-z0-9_.-]+", "_", os.path.basename(output_path))
    tmp_dir = os.path.join(tmp_root, tmp_name)
    os.makedirs(tmp_dir, exist_ok=True)
    for fn in os.listdir(tmp_dir):
        stale_path = os.path.join(tmp_dir, fn)
    ...
    while queue and len(screens_data) < max_screens:
        ...
        for step in parent_macro:
            ...
            if action == "tap":
                ...
                _execute_swipe_step(step)
            elif action in ("launch", "force_stop"):
                pass  # Already handled above

        # Recreate the viewport in which this element was discovered before
        # using its saved coordinates.
        target_scroll_steps = _scroll_steps(element.get("found_at_scroll", 0))
        for step in target_scroll_steps:
            _execute_swipe_step(step)
        ...
        # Dump and scroll-explore the new screen.  The old implementation only
        # explored screen_0, so navigation elements below the first viewport
        # of every child screen were silently missing.
        prefix = f"screen_{len(screens_data)}"
        new_elements = _scroll_explore(prefix, tmp_dir)
        ...
        new_macro = list(parent_macro) + target_scroll_steps + [
