...
def make_vertical_scroll_step(
    screen_w: int,
    screen_h: int,
    wait: float = 0.5,
) -> dict:
    """Return the canonical upward page-scroll used by crawl and replay.

    ``found_at_scroll`` is only meaningful when the crawler and macro player
    use identical swipe geometry.  Keep this helper as the single source of
        ...
        def _explore(screen_id: str, prefix_texts: list[str], depth: int, max_depth: int = 6):
            ...
            for e in screen.elements:
                ...
                for _ in range(e.found_at_scroll):
                    op_macro.append(make_vertical_scroll_step(
